...
class BatchedPerturbationConfig:
    ...
    def is_noop(self) -> bool:
        ...
        Deliberately block-independent: lets callers skip the per-block
        ``is_perturbed(..., self.idx)`` checks, whose ``idx`` guards otherwise
        force torch.compile to specialize every transformer block separately
        (48 blocks -> recompile_limit overflow -> eager fallback).
        """
